...
class InquiryActivity : AppCompatActivity() {
    ...
    private fun initListener() {
        ...
        binding.SendLogText.setOnClickListener {
            sendDatabaseByEmail(this, listOf("log_database", "location_database", "visit_database"))
        }
    ...
    private fun sendDatabaseByEmail(context: Context, databaseNames: List<String>) {
        try {
            ...
            val emailIntent = Intent(Intent.ACTION_SEND_MULTIPLE).apply {
                ...
                putExtra(Intent.EXTRA_EMAIL, arrayOf("vecto.help@gmail.com")) // 받는 사람의 이메일 주소
                putExtra(Intent.EXTRA_SUBJECT, "VECTO_USER_DB_FILE") // 이메일 제목
                putExtra(Intent.EXTRA_TEXT, "${Auth.userId.value.toString()}님의 로그데이터입니다.") // 이메일 본문
                putExtra(Intent.EXTRA_STREAM, fileUris)
                addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION)
            }

            context.startActivity(Intent.createChooser(emailIntent, "메일로 보내기"))
        } catch (e: Exception) {
            e.printStackTrace()
