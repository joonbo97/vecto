...
import com.vecto_example.vecto.RegisterActivity
...
class LoginActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.LoginButton.setOnClickListener {
            ...
            sendLoginRequest(VectoService.LoginRequest(userid, userpw, fcmtoken), "vecto")
        }
                                ...
                                if (e != null) {
                                    ...
                                    UserApiClient.instance.me { user, error ->
                                        if (error != null) {
                                            ...
                                            sendLoginRequest(VectoService.LoginRequest(user.id.toString(), null, fcmtoken), "kakao")
                                        }
                                    }
                                }
                            }
    ...
    private fun sendLoginRequest(loginRequest: VectoService.LoginRequest, provider: String){
        ...
        call.enqueue(object : Callback<VectoService.VectoResponse<String>>{
            override fun onResponse(call: Call<VectoService.VectoResponse<String>>, response: Response<VectoService.VectoResponse<String>>) {
                if (response.isSuccessful) {
                    // 로그인 성공
                    Log.d("VectoLogin", "로그인 성공 : " + response.body())
                    ...
                    if(response.code() == 401){
                        if(provider == "kakao") {//kakao일 때, 로그인에 실패하면 회원가입까지 자동으로 완료.
                            sendRegisterRequest(VectoService.RegisterRequest(loginRequest.userId, null, provider, nickname, null, null), provider)
    ...
    private fun sendRegisterRequest(registerRequest: VectoService.RegisterRequest, provider: String){
        val vectoService = VectoService.create()
        ...
        call.enqueue(object : Callback<VectoService.VectoResponse<String>>{
            override fun onResponse(call: Call<VectoService.VectoResponse<String>>, response: Response<VectoService.VectoResponse<String>>) {
                if (response.isSuccessful) {
                    ...
                    sendLoginRequest(VectoService.LoginRequest(registerRequest.userId, null, fcmtoken), provider)
                } else {
                    // 서버 에러 처리
